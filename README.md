...
This starts NATS + mobility + Sionna in WSL2 and connects to AirSim running on Windows. Flight trajectories are generated by the RL planner at runtime.
...
## RL trajectory planning with Stable-Baselines3

Trajectory planning is now RL-driven by default (no `path*.csv` is required for mission execution). The planner is implemented in the project-level `rl_planner.py` (not under `examples/`).

1. Install dependencies (`stable-baselines3` and `gymnasium` are already listed in `requirements.txt`).
2. In `caviar_config.py`, set the RL method and training hyperparameters:

   - `rl_algorithm = "PPO"` (also supports `A2C` and `DQN`)
   - `rl_total_timesteps`, `rl_learning_rate`, `rl_gamma`

3. Configure model persistence:

   - `rl_model_dir` and `rl_model_name` to save/load SB3 models
   - `rl_force_retrain = True/False` to retrain every run or reuse saved checkpoints
